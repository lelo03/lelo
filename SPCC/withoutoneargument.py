def expand_macro(source_code, macro_definition):
    expanded_code = []
    macro_lines = macro_definition.strip().split('\n')[1:-1]  # Extracting macro lines
    macro_name, macro_body = macro_lines[0].split()[1], macro_lines[1:]  # Extracting macro name and body
...
def calculate_statistics(source_code, macro_calls):
    ...
    num_instructions_in_macro = sum(1 for call in macro_calls for line in call[1:])
